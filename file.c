...
static char user_path[240];
static char save_path[240];
...
static const char* basename_no_leading_slash(const char* name)
{
    if (name && (name[0] == '/' || name[0] == '\\')) return name + 1;
    return name;
}

static void join_path(char* out, size_t out_size, const char* base, const char* name)
{
    const char* clean = basename_no_leading_slash(name);
    if (base == NULL || *base == '\0')
    {
        snprintf(out, out_size, "%s", (clean ? clean : ""));
        return;
    }
    size_t len = strlen(base);
    const char* sep = (len > 0 && (base[len-1] == '/' || base[len-1] == '\\')) ? "" : "/";
    snprintf(out, out_size, "%s%s%s", base, sep, (clean ? clean : ""));
}
...
#elif defined(_WIN32)
    if (save_path[0] != '\0')
        join_path(fullname, sizeof(fullname), save_path, savegamename);
    else if (user_path[0] != '\0')
        join_path(fullname, sizeof(fullname), user_path, savegamename);
    else
        return NULL;
#else
...
#elif defined(_WIN32)
    if (save_path[0] != '\0')
        join_path(fullname, sizeof(fullname), save_path, configname);
    else if (user_path[0] != '\0')
        join_path(fullname, sizeof(fullname), user_path, configname);
    else
...
void set_paths()
{
    ...
    user_path[0] = '\0';
    save_path[0] = '\0';
    char* pref = SDL_GetPrefPath("Asylum", "Asylum");
    if (pref)
    {
        strncpy(user_path, pref, sizeof(user_path) - 1);
    ...
    if (chdir(RESOURCEPATH) == 0)
    {
        ...
        if (user_path[0] != '\0')
        {
            join_path(score_path, sizeof(score_path), user_path, "hiscores");
        }
        else
        {
...
void open_scores()
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        join_path(filename, sizeof(filename), score_path, score_name[i]);
        ...
        if (score_file[i] != NULL) opened_any = 1;
    }
